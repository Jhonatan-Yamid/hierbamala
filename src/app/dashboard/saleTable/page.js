// components/DailySales.js (o donde tengas tu componente DailySales)
"use client";
import { useState, useEffect, useCallback } from "react";
import { FaCashRegister, FaEdit, FaEye } from "react-icons/fa";
...
function DailySales() {
  ...
  // Función para obtener las ventas (reutilizable)
  const fetchSalesData = useCallback(async () => {
    try {
      ...
      // Opcional: setError("Error al cargar las ventas.");
    }
  }, []); // Dependencias vacías para useCallback para que la función sea estable

  useEffect(() => {
    fetchSalesData();
  }, [fetchSalesData]); // Dependencia de fetchSalesData (es estable gracias a useCallback)
  ...
  // NUEVA FUNCIÓN: handleStatusAdvance
  const handleStatusAdvance = async (sale) => {
    let newStatus = "";
    if (sale.status === "en proceso") {
      newStatus = "en mesa"; // Pasa de "en proceso" a "en mesa" (Orden lista)
    } else if (sale.status === "en mesa") {
      newStatus = "pagada"; // Pasa de "en mesa" a "pagada" (Marcar como pagada)
    } else {
      // Si el estado es "pagada" o desconocido, no hacemos nada.
      return; 
    }

    try {
      const res = await fetch(`/api/sale/${sale.id}/status`, {
        method: "PUT", // O PATCH, ambos son válidos para actualización parcial
        headers: {
      ...
      // Vuelve a obtener los datos de las ventas para que la UI se actualice
      // con el nuevo estado y el botón cambie/desaparezca.
      fetchSalesData();
    } catch (error) {
          ...
          <div key={sale.id} className="flex items-center mb-4 cursor-pointer">
            ...
            <div className="flex justify-between w-full">
              <div>
                ...
                {/* Botón condicional según el estado actual */}
    ...
    <div className="flex space-x-3 mb-4">
      <button
        ...
        className={`px-3 py-1 rounded-full text-sm ${
          subTab === "todas" ? "bg-green-700 text-white" : "bg-gray-600 text-gray-200"
        }`}
      >
        Todas
      </button>
      <button
        ...
        className={`px-3 py-1 rounded-full text-sm ${
          subTab === "pendientes" ? "bg-yellow-700 text-white" : "bg-gray-600 text-gray-200"
        }`}
      >
        En proceso / En mesa
      </button>
      <button
        ...
        className={`px-3 py-1 rounded-full text-sm ${
          subTab === "pagadas" ? "bg-blue-700 text-white" : "bg-gray-600 text-gray-200"
        }`}
      >
        Pagadas
      </button>
